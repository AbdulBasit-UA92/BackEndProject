...
<head>
    <title>RISHTON ACADEMY - UPDATE DINNER MONEY</title>
...
$sqlb = "SELECT DinnerMoneyID FROM DinnerMoney";
$dinnermoney = mysqli_query($link, $sqlb);

?>

  <hr> <h1>UPDATE DINNER MONEY</h1> <hr>

        <table >
          <form method="post" action="UpdateDinnerMoney.php">
          <tr class="input">
            <td>
              <label for="dinnermoneyid">Dinner Money ID:</label><br>
              <select name="dinnermoneyid">
              <?php
                while ($row = mysqli_fetch_array(
                        $dinnermoney,MYSQLI_ASSOC)):;
                ?>
                <option value="<?php echo $row["DinnerMoneyID"];?>">
                <?php echo $row["DinnerMoneyID"];?>
                </option>
              <?php
              endwhile;
              ?>
            ...
            <tr class="input">
              <td>
                <label for="amount">Dinner Money Amount:</label><br>
                <input type="number" name="amount" placeholder="Dinner Money Amount" required>
              </td>
              <td>
...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE DinnerMoney SET Amount='$amount', Date='$date' , StudentID = '$studentid'
    WHERE DinnerMoneyID = '$dinnermoneyid'";   
