...
<head>
    <title>RISHTON ACADEMY - DELETE STUDENTS/PARENTS</title>
...
if ($link === false) {
  ...
  $sqld = "SELECT StudentID FROM StudentParent";
  $student = mysqli_query($link, $sqld);

  $sqla = "SELECT ParentID FROM StudentParent";
  $parent = mysqli_query($link, $sqla);
?>



  <hr> <h1>DELETE STUDENTS / PARENTS</h1> <hr>
  <hr> <h1>SELECT STUDENT / PARENT RELATIONSHIP TO DELETE:</h1> <hr>

        <table >
          <form method="post" action="DeleteStudentParent.php">
          <tr class="input">
           <tr class="input">
            <td>
...
if (isset($_POST['submit'])) {
    ...
    $sql = "DELETE FROM StudentParent WHERE StudentID = '$studentid' && ParentID = '$parentid'";  
