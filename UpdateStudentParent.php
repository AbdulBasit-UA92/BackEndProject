...
<head>
    <title>RISHTON ACADEMY - UPDATED STUDENTS/PARENTS</title>
...
if ($link === false) {
  ...
  $sqla = "SELECT StudentID FROM Students";
  $student = mysqli_query($link, $sqla);

  $sqlb = "SELECT ParentID FROM Parents";
  $parent = mysqli_query($link, $sqlb);

  $sqlc = "SELECT StudentID FROM StudentParent";
  $student2 = mysqli_query($link, $sqlc);

  $sqld = "SELECT ParentID FROM StudentParent";
  $parent2 = mysqli_query($link, $sqld);

?>


<hr> <h1>UPDATE STUDENTS/PARENTS RELATIONSHIP</h1> <hr>
<hr> <h3>SELECT STUDENT/PARENT RELATIONSHIP YOU WANT TO UPDATE:</h3> <hr>
<table >
    <form method="post" action="UpdateStudentParent.php">
              <tr class="input">
              <td>
                <label for="studentid2">Student ID:</label><br>
                <select name="studentid2">
                <?php
                  while ($row = mysqli_fetch_array(
                          $student2,MYSQLI_ASSOC)):;
                  ?>
              ...
              <td>
                <label for="parentid2">Parent ID:</label><br>
                <select name="parentid2">
                <?php
                  while ($row = mysqli_fetch_array(
                          $parent2,MYSQLI_ASSOC)):;
                  ?>
          ...
          <tr class="input">
            ...
            <td>
              <label for="studentid">New Student ID:</label><br>
              <select name="studentid">
              <?php
                while ($row = mysqli_fetch_array(
                        $student,MYSQLI_ASSOC)):;
                ?>
                <option value="<?php echo $row["StudentID"];?>">
                <?php echo $row["StudentID"];?>
                </option>
            ...
            <td>
              <label for="parentid">New Parent ID:</label><br>
...
if (isset($_POST['submit'])) {
    ...
    $parentid2 = $_POST['parentid2'];
   
    $sql = "UPDATE StudentParent SET StudentID = '$studentid', ParentID = '$parentid'
    WHERE StudentID = '$studentid2' && ParentID = '$parentid2'";
    if (mysqli_query($link, $sql)) {
      ...
      echo '<script type="text/javascript">
      window.onload = function () { alert("ERROR UPDATED RECORD"); } 
      </script>';
    }
