...
<head>
    <title>RISHTON ACADEMY - UPDATE ASSISTANTS</title>
...
$sqla = "SELECT AssistantID FROM Teaching_assistants";
$assistant = mysqli_query($link, $sqla);

$sqlb = "SELECT TeacherID FROM Teachers";
$teacher = mysqli_query($link, $sqlb);
?>

  <hr> <h1>UPDATE ASSISTANTS</h1> <hr>

        <table>
          <form method="post" action="UpdateAssistants.php">
          <tr class="input">
             <td>
              <label for="assistantid">Assistant ID:</label><br>
              <select name="assistantid">
              <?php
                while ($row = mysqli_fetch_array(
                        $assistant,MYSQLI_ASSOC)):;
                ?>
                <option value="<?php echo $row["AssistantID"];?>">
                <?php echo $row["AssistantID"];?>
                </option>
              <?php
              endwhile;
              ?>
            ...
            <tr class="input">
              <td>
                ...
                <input type="text" name="name" placeholder="Assistant Name"  pattern="[A-Za-z ]{1,255}" required>
              </td>
              <td>
...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE Teaching_assistants SET Name='$name', Address='$address', 
    Phone_number = '$phone', TeacherID = '$teacherid'
